...
describe("Zorsh Edge Cases", () => {
	describe("Numeric Edge Cases", () => {
		test("integer boundaries", () => {
			...
			for (const { schema, max, min } of cases) {
				...
				// Test out of bounds
				const increment = typeof max === "bigint" ? 1n : 1;
				expect(() => schema.serialize(max + increment)).toThrow();
				expect(() => schema.serialize(min - increment)).toThrow();
			}
		});
		...
		test("NaN handling", () => {
			...
			expect(() => b.f64().serialize(NaN)).toThrow();
		});
	...
	describe("Collection Edge Cases", () => {
		test("empty and large collections", () => {
			...
			// Empty collections
			expect(
				vecSchema.deserialize(vecSchema.serialize(new Uint8Array([]))),
			).toEqual(new Uint8Array([]));
			expect(mapSchema.deserialize(mapSchema.serialize(new Map()))).toEqual(
				new Map(),
			);
			expect(setSchema.deserialize(setSchema.serialize(new Set()))).toEqual(
				new Set(),
			);
			...
			expect(
				vecSchema.deserialize(vecSchema.serialize(new Uint8Array(largeVec))),
			).toEqual(new Uint8Array(largeVec));
			expect(
				mapSchema.deserialize(mapSchema.serialize(new Map(largeMap))),
			).toEqual(largeMap);
			expect(
				setSchema.deserialize(setSchema.serialize(new Set(largeSet))),
			).toEqual(largeSet);
		});
		...
		test("complex map keys and values", () => {
			...
			expect(
				complexMapSchema.deserialize(complexMapSchema.serialize(map)),
			).toEqual(map);
		});
	});
